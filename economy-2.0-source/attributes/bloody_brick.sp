...
void HookJarSpawnPost(int entity)
{
    ...
    if  (TF2CustAttr_GetInt(weapon, "proj is brick") > 0)
    {
...
)
{
    if (bIsBrick[inflictor] && IsBrickEntity(inflictor))
    {
        ...
        if (TF2CustAttr_GetInt(weapon, "proj is brick") < 1)
        {
            int brickWeapon = GetPlayerWeaponSlot(attacker, TFWeaponSlot_Secondary);

            // Is this a brick weapon?
            if (IsValidEntity(brickWeapon) && TF2CustAttr_GetInt(brickWeapon, "proj is brick") < 1)
            {
                bleed            = TF2CustAttr_GetFloat(brickWeapon, "brick bleed");
                bleedMin         = TF2CustAttr_GetFloat(brickWeapon, "brick bleed min");
                bleedMax         = TF2CustAttr_GetFloat(brickWeapon, "brick bleed max");
                baseDistance     = TF2CustAttr_GetFloat(brickWeapon, "brick bleed dist");
                minDist          = TF2CustAttr_GetFloat(brickWeapon, "brick bleed dist min");
                additionalDamage = TF2CustAttr_GetFloat(brickWeapon, "full value damage bonus");
            }
        ...
        else
        {
            ...
            bleed            = TF2CustAttr_GetFloat(weapon, "brick bleed");
            bleedMin         = TF2CustAttr_GetFloat(weapon, "brick bleed min");
            bleedMax         = TF2CustAttr_GetFloat(weapon, "brick bleed max");
            baseDistance     = TF2CustAttr_GetFloat(weapon, "brick bleed dist");
            minDist          = TF2CustAttr_GetFloat(weapon, "brick bleed dist min");
            additionalDamage = TF2CustAttr_GetFloat(weapon, "full value damage bonus");
        }
