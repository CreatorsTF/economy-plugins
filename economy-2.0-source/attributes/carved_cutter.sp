...
#define PLUGIN_NAME           "Carved Cutter Attributes"
#define PLUGIN_AUTHOR         "Creators.TF Team"
#define PLUGIN_DESCRIPTION    "Implements the knife leap attributes for the Carved Cutter"
...
{
	char sHUDText[128];
	float flCooldown = TF2CustAttr_GetFloat(GetAttribute(client), "knife leap cooldown");
	if(flCooldown <= 0)
	{
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if(buttons & IN_RELOAD)
	{
		...
		if(iWeapon >= 0)
		{
			...
			float flCooldown = TF2CustAttr_GetFloat(iWeapon, "knife leap cooldown");
			if(flCooldown <= 0 )
			{
				flCooldown = DEFAULT_LEAP_COOLDOWN;
			}
...
void PerformDash(int client, int weapon)
{
	...
	float flCloakDrain = TF2CustAttr_GetFloat(client, "knife leap drain");
	if(flCloakDrain <= 0)
	{
	...
	// make it usable
	float flDistance = TF2CustAttr_GetFloat(weapon, "knife leap distance");
	if(flDistance <= 0)
	{
	...
	float flDistanceVertical = TF2CustAttr_GetFloat(weapon, "knife leap distance vertical");
...
bool HasAttribute(int client)
{
	for (int i = 0; i <= TFWeaponSlot_Melee; i++)
	{
		...
		if(iWeapon > 0 && IsValidEntity(iWeapon) && TF2CustAttr_GetInt(iWeapon, "knife leap") > 0)
		{
			return true;
		}
	}
