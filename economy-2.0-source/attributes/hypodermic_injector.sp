...
#define PLUGIN_NAME           "Hypodermic Injector Attributes"
#define PLUGIN_AUTHOR         "Creators.TF Team - quality, HiGPS"
#define PLUGIN_DESCRIPTION    "Adds the syringe blood mod and other attributes for the Hypodermic Injector."
...
void UpdatePoseParameter(int client, int weapon)
{
	...
	SetEntPropFloat(weapon, Prop_Send, "m_flPoseParameter", float(m_iBlood[client]) / float(m_iBloodCap[client]));
}
...
void ApplyCharge(int client, int victim, int weapon)
{
	...
	int iHealing = TF2CustAttr_GetInt(weapon, "syringe blood mode heal");
	...
	TF2_AddCondition(victim, APPLY_CONDITION, TF2CustAttr_GetFloat(weapon, "syringe blood mode uber"), client);
...
{
	return TF2CustAttr_GetInt(weapon, "syringe blood mode") > 0;
}
