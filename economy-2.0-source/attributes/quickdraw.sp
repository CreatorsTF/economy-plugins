...
public void OnWeaponSwitch(int client, int weapon)
{
	...
	if (TF2CustAttr_GetFloat(weapon, "quickdraw timed damage bonus") > 0.0)
	{
		...
		float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	if (fBonusUntil[attacker] > GetEngineTime())
	{
		...
		if (TF2CustAttr_GetInt(iActiveWeapon, "duel against quickdraw engineers") > 0)
		{
			...
			if (IsValidEntity(iVictimActiveWeapon))
			{
				...
				if (TF2CustAttr_GetInt(iVictimActiveWeapon, "duel against quickdraw engineers") > 0)
				{
					...
					// TODO: When contracts are reimplemented, readd this.
