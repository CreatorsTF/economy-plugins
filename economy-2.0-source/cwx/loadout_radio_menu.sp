 ...
 */
void BuildEquipMenu(StringMap hCustomItems) {
	delete s_EquipMenu;
	...
	StringMapSnapshot itemList = hCustomItems.Snapshot();
	
	for (int i; i < itemList.Length; i++) {
		itemList.GetKey(i, uid, sizeof(uid));
		
		CustomItemDefinition item;
		CWX_GetCustomItemDefinition(uid, item);
...
static int OnLoadoutSlotMenuEvent(Menu menu, MenuAction action, int param1, int param2) {
	switch (action) {
		...
		case MenuAction_Display: {
			...
			char buffer[192];
			FormatEx(buffer, sizeof(buffer), "Creators.TF Weapons [CWX]");
...
static int OnEquipMenuEvent(Menu menu, MenuAction action, int param1, int param2) {
	switch (action) {
		...
		case MenuAction_DrawItem: {
			...
			CustomItemDefinition item;
			if (!CWX_GetCustomItemDefinition(uid, item) || !ItemVisibleInEquipMenu(client, item)) {
