...
bool HasAttributeActive(int client)
{
    ...
    if (IsValidEntity(iActiveWeapon))
    {
        return CEconItems_GetEntityAttributeInteger(iActiveWeapon, "charge shot with secondary") > 0;
    }
    return false;
}
...
bool HasAttribute(int weapon)
{
	...
	{
		return CEconItems_GetEntityAttributeInteger(weapon, "charge shot with secondary") > 0;
	}
	return false;
}
